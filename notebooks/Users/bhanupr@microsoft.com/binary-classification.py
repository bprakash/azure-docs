...
# MAGIC We will use this information to predict if an individual earns >50k a year or <=50K a year.
...
# MAGIC 
# MAGIC Since we will have more than 1 stages of feature transformations, we use a [Pipeline] to tie the stages together.
...
# MAGIC %md
# MAGIC Next, we will use the `VectorAssembler` to combine all the feature columns into a single vector column.
# MAGIC This will include both the numeric columns and the one-hot encoded binary vector columns in our dataset.
...
# MAGIC %md
# MAGIC We finally run our stages as a Pipeline.
# MAGIC This puts the data through all of the feature transformations we described in a single call.
